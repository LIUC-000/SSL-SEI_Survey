...
def train_and_test(encoder, classifier, loss_nll, train_dataloader, val_dataloader, optim_encoder, optim_classifier,
    ...
    current_min_test_loss = 100000000
    for epoch in range(1, epochs + 1):
        train(encoder, classifier, loss_nll, train_dataloader, optim_encoder, optim_classifier, epoch, device, writer,
              freeze_encoder)
        validation_loss = evaluate(encoder, classifier, loss_nll, val_dataloader, epoch, device, writer)
...
def main(k):
    ...
    for i in range(config['iteration']):
        ...
        save_path_classifier = f"./model_weight/classifier_PT_{params['class_start']}-{params['class_end']}_FT_{config_ft['class_start']}-{config_ft['class_end']}_{k}shot_{i}.pth"
        save_path_online_network = f"./model_weight/online_network_PT_{config['trainer']['class_start']}-{config['trainer']['class_end']}_FT_{config_ft['class_start']}-{config_ft['class_end']}_{k}shot_{i}.pth"

        X_train, X_test, Y_train, Y_test = FineTuneDataset_prepared(range(params['class_start'], params['class_end']+1), k) #---------
        ...
        # train
        test_acc = run(train_dataloader, test_dataloader, epochs=params['max_epochs'], device=device,
