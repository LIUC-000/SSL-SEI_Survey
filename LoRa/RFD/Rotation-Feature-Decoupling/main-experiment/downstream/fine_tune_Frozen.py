...
def run(train_dataloader, test_dataloader, epochs, save_path_classifier, device, writer):
    ...
    encoder = Encoder()
    encoder = get_pretrain_encoder(encoder, None, "../pretext/_experiments/Rot_Predict_CVCNN/feature_net_epoch300")
...
def main(k, SEED, test_acc_all, snr):
    ...
    print(f"SEED: {SEED}--------------------------------------------------------")
    set_seed(SEED)
    writer = SummaryWriter(f"./log_finetune/Frozen_PT_0-19_FT_20-29_{k}shot_{SEED}")

    save_path_classifier = f"./model_weight/Frozen_classifier_PT_0-19_FT_20-29_{k}shot_{SEED}.pth"

    X_train, X_test, Y_train, Y_test = FineTuneDataset_prepared(range(20, 30), k, snr, SEED)
...
if __name__ == '__main__':
    ...
    for snr in [-10,-5,0,5,10]:
        ...
        df.to_excel(
        f"test_result/Frozen_PT_0-19_FT_20-29_{k}shot_SNR={snr}.xlsx")
