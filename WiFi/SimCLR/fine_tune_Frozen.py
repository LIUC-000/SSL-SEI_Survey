...
def main(k, SEED, test_acc_all, snr):
    ...
    print(f"SEED: {SEED}--------------------------------------------------------")
    set_seed(SEED)
    writer = SummaryWriter(f"./log_finetune/Frozen_PT_{config['trainer']['ft']}ft_{config['trainer']['class_start']}-{config['trainer']['class_end']}_FT_{config_ft['ft']}ft_{config_ft['class_start']}-{config_ft['class_end']}_{k}shot_{SEED}")

    save_path_classifier = f"./model_weight/Frozen_classifier_PT_{config['trainer']['ft']}ft_{config['trainer']['class_start']}-{config['trainer']['class_end']}_FT_{config_ft['ft']}ft_{config_ft['class_start']}-{config_ft['class_end']}_{k}shot_{SEED}.pth"

    X_train, X_test, Y_train, Y_test = FineTuneDataset_prepared(config_ft['ft'], range(config_ft['class_start'], config_ft['class_end']+1), k, snr, SEED)
...
if __name__ == '__main__':
    ...
    for snr in [-10,-5,0,5,10]:
        ...
        df.to_excel(
        f"test_result/Frozen_PT_{config['trainer']['ft']}ft_{config['trainer']['class_start']}-{config['trainer']['class_end']}_FT_{config_ft['ft']}ft_{config_ft['class_start']}-{config_ft['class_end']}_{k}shot_SNR={snr}.xlsx")
