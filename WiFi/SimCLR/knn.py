...
def clustering_PT():
    ...
    X_train, Y_train = TestDataset_prepared(62, range(0, 10))
    train_dataset = TensorDataset(torch.Tensor(X_train), torch.Tensor(Y_train))
    train_dataloader = DataLoader(train_dataset, batch_size=config_trainer['batch_size'], shuffle=False)
    ...
    # 获取特征图
    X_test_embedding_feature_map, target_test = obtain_embedding_feature_map(online_network, train_dataloader)
