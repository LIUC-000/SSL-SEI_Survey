...
def WiFi_Dataset_slice(ft, classi):
    ...
    for classes in classi:
        for recoder in range(1):
            inputFilename = f'/data/Dataset/KRI-16Devices-RawData/{ft}ft/WiFi_air_X310_{devicename[classes]}_{ft}ft_run{recoder+1}'
...
def TestDataset_prepared(ft, classi):
    ...
    max_value, min_value = Max_Min_PT(62,range(0,10))
    X_test = (X_test - min_value) / (max_value - min_value)
