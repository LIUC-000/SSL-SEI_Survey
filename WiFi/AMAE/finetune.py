...
def main(k, snr, seed):
    ...
    set_seed(seed)
    X_train, X_test, Y_train, Y_test = FineTuneDataset_prepared(conf.ft, range(conf.n_classes[0], conf.n_classes[1]), k, snr, seed)
...
if __name__ == '__main__':
    ...
    for snr in [-10,-5,0,5,10]:
        ...
        df.to_excel(
        f"test_result/Frozen_PT_{conf.ft}ft_0-9_FT_{conf.ft}ft_{conf.n_classes[0]}-{conf.n_classes[1]-1}_{k}shot_SNR={snr}.xlsx")
